...
(define (convince stage)
  (define (light-on txt b)
    (if b (light-gray txt) (t txt)))
...
(define (concrete stage)
  ...
  (define memo-table
    (progressive-table stage (list id1memo app1memo id2memo app2memo) 2
                       (list (angles (scode x ρ₁ σ₂)) (scode 1)
                             (angles (scode (f y) ρ₁ σ₁)) (scode 1)
                             (angles (scode x ρ₅ σ₅)) (scode 2)
                             (angles (scode (f y) ρ₄ σ₄)) (scode 2))
  ...
  (define store-table
    (progressive-table stage (list app1 app1 id1 bindn1 app2 id2 bindn2) 2 #:ghost? #f
                       (list (cond [(< 0 stage app2) (paddr "f₀")]
                                   [(>= stage app2) (paddr "f₀,f₁")]
                                   [else (blank)]) (scode id)
                                                   (paddr "y₀") (cond [(< 0 stage) (scode 1)]
                                                                     [else (blank)])
